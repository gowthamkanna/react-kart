    ...
    const productData = useMemo(() => {
      ...
      if(filterOption.categories.length > 0){
        filterOption.categories.map((filterItem, index) =>{
          productsResult = products.filter((product) => product.ProductCategory._id === filterItem);
          if(index > 0){
      ...
      if(filterOption.keyword !==''){
          productsArray = productsArray.filter((product) => product.Name.toLowerCase().includes(filterOption.keyword.toLowerCase()) || product.Description.toLowerCase().includes(filterOption.keyword.toLowerCase()));
      }
                                       ...
                                       <div className="mb-30 filter-options">
                                          {
                                              categories ? categories.map((item) => (
                                                  <div className="custom-control custom-checkbox mb-3 mt-3" key={item._id}>
                                                      <input type="checkbox" className="custom-control-input" id={item.name} onClick={(e) => handleFilterOption(e) } value={item._id} />&nbsp;
