...
router.post("/add-to-cart", async(req, res) => {
    try {
        ...
        const productExists =  await customerRecord.AddToCart.find((product) => product==req.body.productID);
        ...
        .then(data => {
            res.status(200).send({type: "success" ,message: "Product added to your cart..!", cart: cartArray});
        })
        .catch(err => {
            ...
            res.status(200).send({type: "success" ,message: "Product already in your cart..!"});
        }
    }
    catch(err){
...
router.get("/cart/:id", async(req, res)=> {
    try{
        const cartProducts = await Customer.findById(req.params.id).populate({path : 'AddToCart'});
...
router.post("/remove-cart", async(req, res) => {
    try{
        ...
        updatedCartProduct = cartProductsResult.filter((item) => (item.toString() !== req.body.productID));
        await Customer.findByIdAndUpdate(req.body.userID, {
            AddToCart: updatedCartProduct,
            }, {new: true})
        .then(data => {
            res.status(200).send({type: "success" ,message: "Product removed to your cart..!", cart: data.AddToCart});
        })
        .catch(err => {
